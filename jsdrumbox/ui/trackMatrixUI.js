...
TrackMatrixUI.prototype.doMouseDown = function(event) {
		//must recompute w_bar ?????
	var w_track = document.getElementById("tracksDataTable").rows[0].cells[2].offsetWidth ;
	...
	x = event.clientX - (document.getElementById("tracksDataTable").rows[0].cells[2].offsetLeft      + document.getElementById("tracksDataTable").offsetLeft);
	y = event.clientY - (document.getElementById("tracksDataTable").rows[nbTracks].cells[2].offsetTop
		+ h
...
//
TrackMatrixUI.prototype.setTime = function(time) {
...
TrackMatrixUI.prototype.drawCursor = function(w_bar, currentBarNum, w_step, currentStepInBar) {
	var w_cursor = w_step;
...
TrackMatrixUI.prototype.drawNotes = function(track, w_bar, w_step) {
	this.canvas2d.fillStyle = "#FF8800";
	var w_note = w_step;
	var h_note = this.height/12;
	var y_note = 0;
	for ( var i = 0; i < track.posNotes.length; i++) {
		y_note = this.height - this.height*track.posNotes[i].note.pitch;
...
TrackMatrixUI.prototype.drawBarMarks = function(w_bar, nbBars, w_step, nbStepsPerBar) {
